...
static I NOINLINE jtis(J jtfg,A n,A v,A symtab){F12IP0;CHKAPX(n);CHKAPX(v);CHKAPX1(symtab);
 B ger=0;C *s;
...
F1(jtparse){F12IP1;A z;I stackallo=0;
 ARGCHK1(w);
...
static A nameundco(J jtfg, A name, A y){F12IP0;CHKAPX(name);CHKAPX1(y);
 A locfound;
...
A jtparsea(J jtfg, A *queue, I nwds){F12IP0; /* CHKAPX1(*queue); */ PSTK *stack;
