...
static I NOINLINE jtis(J jtfg,A n,A v,A symtab){F12IP0;CHKAPX(n);CHKAPX(v); // CHKAPX(symtab);
 B ger=0;C *s;
...
static A nameundco(J jtfg, A name, A y){F12IP0;CHKAPX(name);/* CHKAPX(y); */
 A locfound;
...
A jtparsea(J jtfg, A *queue, I nwds){F12IP0; /* CHKAPX(*queue); */ PSTK *stack;
