...
F1(jtbehead ){PLOG1;R drop(one,    w);}
F1(jtcurtail){PLOG1;R drop(num[-1],w);}

F1(jtshift1){PLOG1;R drop(num[-1],over(one,w));}
...
static F2(jttks){PLOG2;PROLOG;A a1,q,x,y,z;B b,c;I an,m,r,*s,*u,*v;P*wp,*zp;
...
static F2(jttk){PLOG2;PROLOG;A y,z;B b=0;C*yv,*zv;I c,d,dy,dz,e,i,k,m,n,p,q,r,*s,t,*u;
...
F2(jttake){PLOG2;A s,t;D*av,d;I acr,af,ar,n,*tv,*v,wcr,wf,wr;
 RZ(a&&w);
...
F2(jtdrop){PLOG2;A s;I acr,af,ar,d,m,n,*u,*v,wcr,wf,wr;
 RZ((a=vib(a))&&w);
 ar=AR(a); acr=jt->rank?jt->rank[0]:ar; af=ar-acr; 
 wr=AR(w); wcr=jt->rank?jt->rank[1]:wr; wf=wr-wcr; jt->rank=0;
...
static F1(jtrsh0){PLOG1;A x,y;I wcr,wf,wr,*ws;
 wr=AR(w); wcr=jt->rank?jt->rank[1]:wr; wf=wr-wcr; jt->rank=0;
 ws=AS(w);
