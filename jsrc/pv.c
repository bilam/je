...
static F1(jtvtokens){PLOG1;A t,*y,z;I n,*s;TA*x;
...
static F1(jtcfn){PLOG1;I j; R !AR(w)&&INT&AT(w)&&(j=*AV(w),-9<=j&&j<=9)?FCONS(w):qq(w,ainf);}
     /* constant function with value w */

static F1(jttine){PLOG1;V*v; R w&&jt->tmonad&&(v=VAV(w),CP==v->f&&RT==v->h)?v->g:w;}
...
static F1(jtvfinal){PLOG1;I i;V*u,*v;
 RZ(w);
 if(!(VERB&AT(w)))R w;
 v=VAV(w);
...
F1(jttparse){PLOG1;A*s,t,x;C d;I b,*c,e,i,j,k,m,n;TA*stack;
 RZ(w);
...
F1(jtvtrans){PLOG1;PROLOG;A local,y,z=0;B tmonad,tsubst;I c,i;TA ttab[NTTAB],*ttab0;
