...
static F1(jtcfd){PLOG1;A b,q,x,z,*zv;B*bv;I c,i,j,n,*qv,*u,*v,zn;
 RZ(w);
 if(c=1&&INT&AT(w)){
  n=AN(w); v=AV(w);
...
F1(jtcdot1){PLOG1;F1RANK(1,jtcdot1,0); R BOX&AT(w)?dfc(ord(raze(w)),w):cfd(w);}

F2(jtcdot2){PLOG2;A p;
 F2RANK(1,RMAX,jtcdot2,0);
 RZ(p=BOX&AT(a)?dfc(IC(w),a):pfill(IC(w),a));
 R AR(w)?from(p,w):w;
}

F1(jtpparity){PLOG1;A x,y,z;B p,*u;I i,j,k,m,n,r,*s,*v,*zv;
 RZ(x=cvt(INT,w));
...
static F1(jtdfr){PLOG1;A z;I c,d,i,j,m,n,*v,*x;
 RZ(w);
...
static F1(jtrfd){PLOG1;A z;I j,k,n,r,*s,*x;
 RZ(z=ca(w)); x=AV(z);
 r=AR(w); s=AS(w); 
