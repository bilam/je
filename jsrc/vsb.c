...
static F1(jtsbunbox){PLOG1;A*wv,x,z;B c2;I i,m,n,wd;SB*zv;
 RZ(w);
...
static F1(jtsbunind){PLOG1;A z;I j,n,*zv;
 RZ(z=cvt(INT,w));
 zv=AV(z); n=jt->sbun;
...
F1(jtsborder){PLOG1;A z;I n,*zv;SB*v;
 RZ(w);
 n=AN(w); v=SBAV(w);
...
static F1(jtsbbox){PLOG1;A z,*zv;C*s;I n;SB*v;SBU*u;
 RZ(w);
 n=AN(w); v=SBAV(w);
 ASSERT(!n||SBT&AT(w),EVDOMAIN);
...
static F1(jtsbhashstat){PLOG1;A z;I j,k,n,p,*zv;SBU*v;
...
static F1(jtsbcheck){PLOG1;R sbcheck1(sc(jt->sbun),sc(jt->sbsn),jt->sbu,jt->sbs,jt->sbh,sc(ROOT),sc(FILLFACTOR),sc(GAP));}

static F1(jtsbsetdata){PLOG1;A h,s,u,*wv,x;I wd;
 RZ(w);
...
static F1(jtsbgetdata){PLOG1;A z,*zv;
