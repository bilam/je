...
static DF1(jtcork1){PLOG1;DECLFGH;PROLOG;A z;  CAP1; EPILOG(z);}
static DF2(jtcork2){PLOG2;DECLFGH;PROLOG;A z;  CAP2; EPILOG(z);}
static DF1(jtfolk1){PLOG1;DECLFGH;PROLOG;A z; FOLK1; EPILOG(z);}
static DF2(jtfolk2){PLOG2;DECLFGH;PROLOG;A z; FOLK2; EPILOG(z);}
...
static DF1(jtcorx1){PLOG1;DECLFGH;PROLOG;A z; if(cap(fs))RZ(z=df1(  w,folk(ds(CCAP),gs,hs))) else FOLK1; EPILOG(z);}
static DF2(jtcorx2){PLOG2;DECLFGH;PROLOG;A z; if(cap(fs))RZ(z=df2(a,w,folk(ds(CCAP),gs,hs))) else FOLK2; EPILOG(z);}
     /* f g h where f may be [: */

static DF1(jtnvv1){PLOG1;DECLFGH;PROLOG; EPILOG(CALL2(g2,fs,CALL1(h1,  w,hs),gs));}
static DF2(jtnvv2){PLOG2;DECLFGH;PROLOG; EPILOG(CALL2(g2,fs,CALL2(h2,a,w,hs),gs));}

static DF2(jtfolkcomp){PLOG2;DECLFGH;PROLOG;A z;AF f;
 RZ(a&&w);
 if(f=atcompf(a,w,self))z=f(jt,a,w,self); else if(cap(fs))CAP2 else FOLK2;
 EPILOG(z);
}

static DF2(jtfolkcomp0){PLOG2;DECLFGH;PROLOG;A z;AF f;D oldct=jt->ct;
 RZ(a&&w);
 jt->ct=0;
 if(f=atcompf(a,w,self))z=f(jt,a,w,self); else if(cap(fs))CAP2 else FOLK2;
 jt->ct=oldct;
...
static DF1(jtcharmapa){PLOG1;V*v=VAV(self); R charmap(w,VAV(v->h)->f,v->f);}
static DF1(jtcharmapb){PLOG1;V*v=VAV(self); R charmap(w,VAV(v->f)->f,VAV(v->h)->f);}
...
static DF1(taa){PLOG1;TDECL;A t=df1(w,fs); ASSERT(!t||AT(t)&NOUN+VERB,EVSYNTAX); R df1(t,gs);}
static DF1(tvc){PLOG1;TDECL; R df2(fs,w,gs);}  /* also nc */
static DF1(tcv){PLOG1;TDECL; R df2(w,gs,fs);}  /* also cn */


static CS1(jthook1, CALL2(f2,w,CALL1(g1,w,gs),fs))
static CS2(jthook2, CALL2(f2,a,CALL1(g1,w,gs),fs))

static DF1(jthkiota){PLOG1;DECLFG;A a,e;I n;P*p;
 RZ(w);
 n=IC(w);
 if(SB01&AT(w)&&1==AR(w)){
...
static DF2(jthkfrom){PLOG2;R from(a,gah(1L,w));}     /* special code for ({ ,) */
static DF2(jthktake){PLOG2;R take(a,gah(1L,w));}     /* special code for ({.,) */
static DF2(jthkdrop){PLOG2;R drop(a,gah(1L,w));}     /* special code for (}.,) */

static DF1(jthkindexofmaxmin){PLOG1;D*du,*dv;I*iu,*iv,n,t,*wv,z=0;V*sv;
 RZ(w&&self);
 n=AN(w); t=AT(w);
...
F2(jthook){PLOG2;AF f1=0,f2=0;C c,d,e,id;I flag=0;V*u,*v;
 RZ(a&&w);
 switch(BD(AT(a),AT(w))){
