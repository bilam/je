...
F1(jtfxeach){PLOG1;R every(w,0L,jtfx);}

static DF1(jtcon1){PLOG1;A h,*hv,*x,z;V*sv;
 PREF1(jtcon1);
...
static DF2(jtcon2){PLOG2;A h,*hv,*x,z;V*sv;
 PREF2(jtcon2);
 sv=VAV(self); h=sv->h; hv=AAV(h);
 GA(z,BOX,AN(h),AR(h),AS(h)); x=AAV(z);
...
static DF1(jtinsert){PLOG1;A f,hs,*hv,z;AF*hf;I j,k,m,n,old;
 RZ(w);
...
F2(jttie){PLOG2;RZ(a&&w); R over(VERB&AT(a)?arep(a):a,VERB&AT(w)?arep(w):w);}
...
static DF1(jtcase1a){PLOG1;A g,h,*hv,k,t,u,w0=w,x,y,*yv,z;B b;I r,*xv;V*sv;
 RZ(w);
 r=AR(w);
...
static DF1(jtcase1b){PLOG1;A h,u;V*sv;
 sv=VAV(self); h=sv->h;
...
static DF1(jtcase1){PLOG1;A h,*hv;B b;I r,wr;V*sv;
 RZ(w);
 sv=VAV(self);
...
static DF2(jtcase2){PLOG2;A u;V*sv;
 PREF2(jtcase2);
 sv=VAV(self);
...
static F2(jtgerfrom){PLOG2;A*av,*v,z;I ad,n;
...
static DF1(jtgav1){PLOG1;DECLF;A ff,*hv=AAV(sv->h);I d;
 d=fdep(hv[1]); FDEPINC(d); ff=df1(df1(w,hv[1]),ds(sv->id)); FDEPDEC(d);
 R df1(df1(w,hv[2]),ff);
}

static DF2(jtgav2){PLOG2;DECLF;A ff,*hv=AAV(sv->h);I d;
...
static DF1(jtgf1){PLOG1;A h=VAV(self)->h; R df1(  w,*AAV(h));}
static DF2(jtgf2){PLOG2;A h=VAV(self)->h; R df2(a,w,*AAV(h));}
