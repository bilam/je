...
F1(jtsiinfo){PLOG1;A z,*zv;DC d;I c=5,n,*s;
 ASSERTMTV(w);
...
DF2(jtdbunquote){PLOG2;A t,z;B b=0,s;DC d;I i;V*sv;
 sv=VAV(self); t=sv->f; 
...
F1(jtdbq){PLOG1;ASSERTMTV(w); R sc(jt->dbuser);}
     /* 13!:17 debug flag */

F1(jtdbrun ){PLOG1;ASSERTMTV(w); jt->dbsusact=SUSRUN;  R mtm;}
     /* 13!:4  run again */

F1(jtdbnext){PLOG1;ASSERTMTV(w); jt->dbsusact=SUSNEXT; R mtm;}
     /* 13!:5  run next */

F1(jtdbret ){PLOG1;RZ(w); jt->dbsusact=SUSRET; jt->dbresult=ra(w); R mtm;}
     /* 13!:6  exit with result */

F1(jtdbjump){PLOG1;RE(jt->dbjump=i0(w)); jt->dbsusact=SUSJUMP; R mtm;}
     /* 13!:7  resume at line n (return result error if out of range) */

static F2(jtdbrr){PLOG2;DC d;
 RE(0);
...
F1(jtdbrr1 ){PLOG1;R dbrr(0L,w);}   /* 13!:9   re-run with arg(s) */
F2(jtdbrr2 ){PLOG2;R dbrr(a, w);}

F1(jtdbtrapq){PLOG1;ASSERTMTV(w); R jt->dbtrap?jt->dbtrap:mtv;}   
     /* 13!:14 query trap */

F1(jtdbtraps){PLOG1;RZ(w=vs(w)); fa(jt->dbtrap); jt->dbtrap=AN(w)?ra(w):0L; R mtm;}
     /* 13!:15 set trap */
