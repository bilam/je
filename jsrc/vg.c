...
F2(jtgrade1p){PLOG2;PROLOG;A x,z;I n,*s,*xv,*zv;
...
F1(jtgr1){PLOG1;PROLOG;A z;I c,f,m,n,r,*s,t,wr,zn;
 RZ(w);
...
F1(jtgrade1 ){PLOG1;GBEGIN( 1,-1); RZ(   w); z=SPARSE&AT(w)?grd1sp(  w):gr1(  w); GEND(z);}
F1(jtdgrade1){PLOG1;GBEGIN(-1, 1); RZ(   w); z=SPARSE&AT(w)?grd1sp(  w):gr1(  w); GEND(z);}
F2(jtgrade2 ){PLOG2;GBEGIN( 1,-1); RZ(a&&w); z=SPARSE&AT(w)?grd2sp(a,w):gr2(a,w); GEND(z);}
F2(jtdgrade2){PLOG2;GBEGIN(-1, 1); RZ(a&&w); z=SPARSE&AT(w)?grd2sp(a,w):gr2(a,w); GEND(z);}
...
F2(jtordstat){PLOG2;A q,t=0;I c,i=0,j,m,m0,m1,n,qn=53,*qv,wt;
 RZ(a&&w);
 n=AN(w); wt=AT(w);
...
F2(jtordstati){PLOG2;A t;I j,n,wt;
 RZ(a&&w);
 n=AN(w); wt=AT(w);
