...
F1(jtlevel1){PLOG1;RZ(w); R sc(level(w));}

F1(jtbox0){PLOG1;R irs1(w,0L,0L,jtbox);}

F1(jtbox){PLOG1;A y,z,*zv;C*wv,*yv;I f,k,m,n,r,wr,*ws; 
 RZ(w);
...
F1(jtboxopen){PLOG1;RZ(w); R AN(w)&&BOX&AT(w)?rat(w):box(w);}

F2(jtlink){PLOG2;RZ(a&&w); R over(box(a),AN(w)&&AT(w)&BOX?rat(w):box(w));}
...
F1(jtope){PLOG1;PROLOG;A cs,*v,y,z;B b,c,h=1;C*x;I d,i,k,m,n,*p,q=RMAX,r=0,*s,t=0,*u,zn;
 RZ(w);
...
F1(jtraze){PLOG1;A*v,y,*yv,z,*zv;B b,zb;C*zu;I d,i,k,m=0,n,q,r=1,*s=0,t=0,yt;
 RZ(w);
...
F1(jtrazeh){PLOG1;A*wv,y,z;C*xv,*yv,*zv;I c=0,ck,dk,i,k,n,p,r,*s,t,wd;
 RZ(w);
...
F2(jtrazefrom){PLOG2;A*wv,y,z;B b;C*v,*vv;I an,c,d,i,j,k,m,n,r,*s,t,*u,wn;
 RZ(a&&w);
 an=AN(a); wn=AN(w);
