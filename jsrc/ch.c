...
static DF2(jthgeom2){PLOG2;PROLOG;A h,*hv,t,z;B b;I an,*av,j,n;V*sv=VAV(self);
 RZ(a&&w);
...
static DF1(jthgeom1){PLOG1;R hgeom2(sc(IMAX),w,self);}

static F2(jtcancel){PLOG2;A c,d,f,x,y;
 f=eval("#/.~");
...
F2(jthgeom){PLOG2;A c,d,h=0;B p,q;I at,wt;
 RZ(a&&w);
...
F1(jthgdiff){PLOG1;A*hv,p,q,x,y;V*v=VAV(w);
 ASSERTNN(v->f,v->g);
 hv=AAV(v->h); 
...
DF1(jthgcoeff){PLOG1;PROLOG;A c,d,h,*hv,y,z;B b;I j,n,pn,qn,*v;V*sv=VAV(self);
 RZ(w=vi(w)); v=AV(w); 
