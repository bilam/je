...
static F1(jtfpolyc){PLOG1;A b;B*bv;I m,n;
...
static F1(jtfnegate){PLOG1;V*v; RZ(w); v=VAV(w); R CAT==v->id&&CMINUS==ID(v->f)?v->g:atop(ds(CMINUS),w);}

static F2(jtfplus){PLOG2;
 RZ(a&&w);
...
static F2(jtftymes){PLOG2;A x,y;B b,c;I k;
 RZ(a&&w);
...
static F1(jtdpoly){PLOG1;A c,e,x;I n,t;
 RZ(w);
 n=AN(w); t=AT(w);
...
static F1(jtipoly){PLOG1;A b,c,e,p=0,q=0,x;I n,t;
 RZ(w);
 n=AN(w); t=AT(w);
 ASSERT(!n||t&NUMERIC+BOX,EVDOMAIN);
...
static F1(jticube){PLOG1;R atco(eval("* =/~@(i.@$)"),w);}

static F1(jtdiffamp0){PLOG1;A f,g,h,x,y;B nf,ng;C id;V*v;
 RZ(w);
 v=VAV(w);
...
static F1(jtdiff0){PLOG1;A df,dg,dh,f,g,h,x,y,z;B b,nf,ng,vf,vg;C id;I m,p,q;V*v;
 RZ(w);
 v=VAV(w); id=v->id;
...
static F1(jtintgamp0){PLOG1;A f,g,h,x,y;B nf,ng;C id;V*v;
 RZ(w);
 v=VAV(w);
 f=v->f; nf=1&&NOUN&AT(f);
 g=v->g; ng=1&&NOUN&AT(g);
...
static F2(jtintgatop){PLOG2;A df,f=a,g=w,q,x,y;I m,n;V*v;
 RZ(a&&w);
 m=ispoly(f);
 n=ispoly(g);
...
static F2(jtintgtymes){PLOG2;A f=a,g=w;
 RZ(a&&w);
 R 0;
}    /* integral of a * w */

static F1(jtintg0){PLOG1;A df,dh,f,g,h;B nf,ng,vf,vg;C id;I m,n,p,q;V*fv,*gv,*v;
 RZ(w);
 id=ID(w); v=VAV(w);
...
static DF1(jtddot1){PLOG1;V*v=VAV(self); R df1(w,ddot(fix(v->f),v->g));}

F2(jtddot){PLOG2;A x,*xv,y,z;AF f;I j,n,p,q,r,*wv;
 RZ(a&&w);
...
static F1(jtdiffamp){PLOG1;A f,g,h,x,y;B nf,ng;V*v;
 RZ(w);
 v=VAV(w);
 f=v->f; nf=1&&NOUN&AT(f);
 g=v->g; ng=1&&NOUN&AT(g);
...
static F1(jtdiff){PLOG1;A df,dh,f,g,h,z;B nf,ng,vf,vg;C id;I r;V*v;
 RZ(w);
...
static F1(jtintg){PLOG1;ASSERT(0,EVNONCE);}
...
static DF2(jtsslope){PLOG2;A fs,f0,p,y,z,*zv;I m,n,r,t;V*sv=VAV(self);
 PREF2(jtsslope);
 fs=sv->f; m=*AV(sv->g);
...
static DF1(jtderiv1){PLOG1;A e,ff,fs,gs,s,t,z,*zv;I*gv,d,n,*tv;V*v;
 PREF1(jtderiv1);
