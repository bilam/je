...
F1(jttally ){PLOG1;A z; RZ(w); z=sc(IC(w));            R AT(w)&XNUM+RAT?xco1(z):z;}
F1(jtshapex){PLOG1;A z; RZ(w); z=vec(INT,AR(w),AS(w)); R AT(w)&XNUM+RAT?xco1(z):z;}
F1(jtshape ){PLOG1;RZ(w); R vec(INT,AR(w),AS(w));}

F1(jtravel){PLOG1;A a,c,q,x,y,y0,z;B*b,d;I f,j,m,n,r,*u,*v,wr,*ws,wt,*yv;P*wp,*zp;
 RZ(w); 
...
F1(jttable){PLOG1;A z;I f,r,*s,wr,*ws,wt;
 RZ(w);
...
F2(jtleft2 ){PLOG2;R lr2(1,a,w);}
F2(jtright2){PLOG2;R lr2(0,a,w);}

F1(jtright1){PLOG1;R rat(w);}

F1(jtiota){PLOG1;A z;I m,n,*v;
 F1RANK(1,jtiota,0);
...
F1(jtjico1){PLOG1;A y,z;B b;D d,*v;I c,m,n; 
 F1RANK(0,jtjico1,0);
...
DF1(jtnum1){PLOG1;RZ(   w&&self); R VAV(self)->h;}
DF2(jtnum2){PLOG2;RZ(a&&w&&self); R VAV(self)->h;}

F2(jtfromr  ){PLOG2;R irs2(a,w,0L,RMAX,1L,jtfrom  );}
F2(jtrepeatr){PLOG2;R irs2(a,w,0L,RMAX,1L,jtrepeat);}

A jttaker(J jt,I n,A w){R irs2(sc(n),w,0L,RMAX,1L,jttake);}
A jtdropr(J jt,I n,A w){R irs2(sc(n),w,0L,RMAX,1L,jtdrop);}

F1(jticap){PLOG1;A a,e;I n;P*p;
 F1RANK(1,jticap,0);
 n=IC(w);
 if(SB01&AT(w)){
