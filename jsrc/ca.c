...
static DF1(jtonf1){PLOG1;PROLOG;DECLFG;A z;I flag=sv->flag,m=jt->xmode;
 PREF1(jtonf1);
...
static DF2(jtuponf2){PLOG2;PROLOG;DECLFG;A z;I flag=sv->flag,m=jt->xmode;
 RZ(a&&w);
 if(primitive(gs))if(flag&VFLR)jt->xmode=XMFLR; else if(flag&VCEIL)jt->xmode=XMCEIL;
...
static DF2(jtmodpow2){PLOG2;A h;B b,c;I at,m,n,wt,x,z;
 PREF2(jtmodpow2);
 h=VAV(self)->h; 
...
static DF1(jtmodpow1){PLOG1;A g=VAV(self)->g; R rank2ex(VAV(g)->f,w,self,0L,0L,jtmodpow2);}
...
static DF2(on2){PLOG2;PROLOG;DECLFG;A ga,gw,z; 
 PREF2(on2); 
 gw=CALL1(g1,w,gs);
 ga=CALL1(g1,a,gs);
 z=CALL2(f2,ga,gw,fs); 
...
static DF2(atcomp){PLOG2;AF f;
 RZ(a&&w); 
 f=atcompf(a,w,self); 
 R f?f(jt,a,w,self):upon2(a,w,self);
}

static DF2(atcomp0){PLOG2;A z;AF f;D oldct=jt->ct;
 RZ(a&&w);
 f=atcompf(a,w,self);
...
F2(jtatop){PLOG2;A f,g,h=0,x;AF f1=on1,f2=jtupon2;B b=0,j;C c,d,e;I flag=0,m=-1;V*av,*wv;
 ASSERTVV(a,w);
 av=VAV(a); c=av->id;
 wv=VAV(w); d=wv->id;
 switch(c){
...
F2(jtatco){PLOG2;A f,g;AF f1=on1,f2=jtupon2;B b=0;C c,d,e;I flag=0,j,m=-1;V*av,*wv;
 ASSERTVV(a,w);
...
F2(jtampco){PLOG2;AF f1=on1;C c,d;I flag=0;V*wv;
 ASSERTVV(a,w);
...
static DF1(withl){PLOG1;DECLFG; R jt->rank?irs2(fs,w,gs,AR(fs),jt->rank[1],g2):CALL2(g2,fs,w,gs);}
static DF1(withr){PLOG1;DECLFG; R jt->rank?irs2(w,gs,fs,jt->rank[1],AR(gs),f2):CALL2(f2,w,gs,fs);}

static DF1(ixfixedleft  ){PLOG1;V*v=VAV(self); R indexofprehashed(v->f,w,v->h);}
static DF1(ixfixedright ){PLOG1;V*v=VAV(self); R indexofprehashed(v->g,w,v->h);}

static DF1(ixfixedleft0 ){PLOG1;A z;D old=jt->ct;V*v=VAV(self); 
 jt->ct=0.0; z=indexofprehashed(v->f,w,v->h); jt->ct=old; 
 R z;
}

static DF1(ixfixedright0){PLOG1;A z;D old=jt->ct;V*v=VAV(self); 
 jt->ct=0.0; z=indexofprehashed(v->g,w,v->h); jt->ct=old; 
 R z;
}

static DF2(with2){PLOG2;R df1(w,powop(self,a));}

F2(jtamp){PLOG2;A h=0;AF f1=on1,f2=on2;B b;C c,d=0;D old=jt->ct;I flag=0,mode=-1,p,r;V*u,*v;
 RZ(a&&w);
 switch(CONJCASE(a,w)){
