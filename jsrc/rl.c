...
static F1(jtltiea){PLOG1;A t,*v,*wv,x,y;B b;C c;I n,wd;
 RZ(w);
...
static F1(jtltieb){PLOG1;A pt,t,*v,*wv,x,y;B b;C c,*s;I wd,n;
 RZ(w);
...
static F1(jtlsh){PLOG1;R over(thorn1(shape(w)),spellout(CDOLLAR));}

static F1(jtlshape){PLOG1;I r,*s;
 RZ(w);
 r=AR(w); s=AS(w);
...
static F1(jtlchar){PLOG1;A y;B b,p=1,r1;C c,d,*u,*v;I j,k,m,n;
 RZ(w);
...
static F1(jtlbox){PLOG1;A p,*v,*vv,*wv,x,y;B b=0;I n,wd;
 RZ(w);
...
static F1(jtlnum1){PLOG1;A z;I t;
 RZ(w);
 t=AT(w);
...
static F1(jtlnum){PLOG1;A b,d,t,*v,y;B p;I n;
 RZ(t=ravel(w));
 n=AN(w);
 if(7<n||1<n&&1<AR(w)){
...
static F1(jtlsparse){PLOG1;A a,e,q,t,x,y,z;B ba,be,bn;I j,r,*v;P*p;
 RZ(w);
...
static F1(jtlnoun0){PLOG1;A s,x;B r1;
 RZ(w);
...
static F2(jtlinsert){PLOG2;A*av,f,g,h,t,t0,t1,t2,*u,y;B b,ft,gt,ht,vb;C c,id;I ad,n;V*v;
 RZ(a&&w);
...
static F1(jtlcolon){PLOG1;A*v,x,y;C*s,*s0;I m,n;
 RZ(y=unparsem(one,w));
...
static F1(jtlrr){PLOG1;A fs,gs,hs,t,*tv;C id;I fl,m;V*v;
 RZ(w);
