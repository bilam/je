...
static DF1(cons1a){PLOG1;R VAV(self)->f;}

static DF2(cons2a){PLOG2;R VAV(self)->f;}

static DF1(cons1){PLOG1;V*sv=VAV(self);
 RZ(w);
 R rank1ex(w,self,efr(AR(w),*AV(sv->h)),cons1a);
}

static DF2(cons2){PLOG2;V*sv=VAV(self);I*v=AV(sv->h);
 RZ(a&&w);
 R rank2ex(a,w,self,efr(AR(a),v[1]),efr(AR(w),v[2]),cons2a);
}

static DF1(rank1i){PLOG1;DECLF;A h=sv->h;I*v=AV(h); R irs1(w,fs,*v,f1);}

static DF2(rank2i){PLOG2;DECLF;A h=sv->h;I*v=AV(h); R irs2(a,w,fs,v[1],v[2],f2);}

static DF1(rank1){PLOG1;DECLF;A h=sv->h;I m,*v=AV(h),wr;
 RZ(w);
 wr=AR(w); m=efr(wr,v[0]);
 R m<wr?rank1ex(w,fs,m,f1):CALL1(f1,w,fs);
}

static DF2(rank2){PLOG2;DECLF;A h=sv->h;I ar,l,r,*v=AV(h),wr;
 RZ(a&&w);
...
F2(jtqq){PLOG2;A h,t;AF f1,f2;D*d;I flag,*hv,n,r[3],*v;
 RZ(a&&w);
