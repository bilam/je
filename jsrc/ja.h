...
#define fauxvirtualcommon(z,v,w,r,c,err) {if(likely((r)<=4)){z=ABACK(w); AK((A)(v))=(CAV(w)-(C*)(v)); AT((A)(v))=AT(w); AR((A)(v))=(RANKT)(r); APINIT((A)v,APX(w)); /* AORIGIN((A)v)=AORIGIN(w); */ z=AFLAG(w)&AFVIRTUAL?z:(w); \
                              AFLAG((A)(v))=AFVIRTUAL|AFUNINCORPABLE|(AFLAG(z)&AFPRISTINE)|(AT(w)&TRAVERSIBLE); ABACK((A)(v))=z; z=(A)(v); ACFAUX(z,(c));} \
                              else{if(unlikely((z=virtual((w),0,(r)))==0))err; AFLAGORLOCAL(z,AFUNINCORPABLE) APINIT(z,APX(w)); /* AORIGIN(z)=AORIGIN(w); */ if((c)!=ACUC1)ACINIT(z,(c))} }
