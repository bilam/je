...
static F1(jtfacit){PLOG1;A c;V*u,*v;
 RZ(c=coeff(w));
...
static F2(jttayinv){PLOG2;A y;I m,*v;
 RZ(a&&w);
 RZ(y=vip(w)); v=AV(y);
...
static DF1(jttayrecip){PLOG1;A f=VAV(self)->f; R from(w,head(tayinv(VAV(f)->g,w)));}
     /* %@f t. w */

static DF1(jttaydiv){PLOG1;A c,f,ft,h,ht,y;I j,m,*u;V*v;
...
static DF1(jttaysqrt){PLOG1;A f;I m,*v;
 f=VAV(self)->f;
 RZ(w=vip(w)); v=AV(w);
 m=0; DO(AN(w), m=MAX(m,v[i]);); ++m;
 ASSERT(0,EVNONCE);
...
static F1(jttayfolk){PLOG1;A c,d,f,ft,g,h,ht,pp;B b;V*v=VAV(w);
...
static F1(jttaysum){PLOG1;I n;V*v=VAV(w);
 switch(ID(w)){
...
static F1(jttayatop){PLOG1;A c,d,e,f,ft,g,gt,h;I k,m,n;V*v=VAV(w);
 f=v->f; g=v->g;
 switch(ID(f)){
...
static DF1(jttcoh1){PLOG1;R hgcoeff(w,VAV(self)->f);}

F1(jttdot){PLOG1;A f,g,h;B nf,ng;C id;V*v;
 RZ(w);
...
F1(jttco){PLOG1;A f,g;B nf,ng;C id;V*v;
 RZ(w);
