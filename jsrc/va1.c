...
F1(jtfloor1){PLOG1;R va1(w,CFLOOR);}
F1(jtceil1 ){PLOG1;R va1(w,CCEIL );}
F1(jtconjug){PLOG1;R va1(w,CPLUS );}
F1(jtsignum){PLOG1;R va1(w,CSTAR );}
F1(jtsqroot){PLOG1;R va1(w,CSQRT );}
F1(jtexpn1 ){PLOG1;R va1(w,CEXP  );}
F1(jtlogar1){PLOG1;R va1(w,CLOG  );}
F1(jtmag   ){PLOG1;R va1(w,CSTILE);}
F1(jtfact  ){PLOG1;R va1(w,CBANG );}
F1(jtpix   ){PLOG1;RZ(w); R XNUM&AT(w)&&(jt->xmode==XMFLR||jt->xmode==XMCEIL)?va1(w,CCIRCLE):tymes(pie,w);}
