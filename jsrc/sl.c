...
F1(jtlocsizeq){PLOG1;I*v; ASSERTMTV(w); v=jt->locsize; R v2(v[0],v[1]);}
     /* 9!:38 default locale size query */

F1(jtlocsizes){PLOG1;I p,q,*v;
 RZ(w);
...
F1(jtlocnc){PLOG1;A*wv,y,z;C c,*u;I i,m,n,wd,*zv;
 RZ(vlocnl(0,w));
...
static F1(jtlocnlx){PLOG1;A*pv,y,*yv,z;B*wv;C s[20];I m=0,n=0,*nv;
...
F1(jtlocnl1){PLOG1;memset(jt->nla,C1,256); R locnlx(w);}
    /* 18!:1 locale name list */

F2(jtlocnl2){PLOG2;UC*u;
 RZ(a&&w);
...
F1(jtlocpath1){PLOG1;A g; F1RANK(0,jtlocpath1,0); RZ(g=locale(1,w)); R LOCPATH(g);}
     /* 18!:2  query locale path */

F2(jtlocpath2){PLOG2;A g,x;
...
static F2(jtloccre){PLOG2;A g,y;C*s;I n,p,*u;L*v;
 RZ(a&&w);
...
static F1(jtloccrenum){PLOG1;C s[20];I k=jt->stmax,p;
 RZ(w);
...
F1(jtlocname){PLOG1;A g=jt->global;
 ASSERTMTV(w);
 ASSERT(g,EVLOCALE);
...
F1(jtlocmap){PLOG1;A g,q,x,y,*yv,z,*zv;I c=-1,d,j=0,m,*qv,*xv;
 RZ(w);
 ASSERT(!AR(w),EVRANK);
...
F1(jtlocexmark){PLOG1;A g,*pv,*wv,y,z;B b,c,*zv;C*u;I i,j,m,n,*nv,wd;L*v;
 RZ(vlocnl(1,w));
 n=AN(w); wv=AAV(w); wd=(I)w*ARELATIVE(w);
