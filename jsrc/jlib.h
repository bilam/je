...
#if NORMAHX==0
  I p0[NORMAHN],k,flag,m,t,c,n,r,s[1];
#elif NORMAHX==1
  I k,p1[NORMAHN],flag,m,t,c,n,r,s[1];
#else
...
CDPROC J _stdcall JInit2(C*libpath);               /* init instance with libpath */
...
CDPROC void _stdcall JInterrupt(J jt);             /* signal interrupt */
...
typedef void  (_stdcall *JInterruptType)(J);
...
typedef int   (_stdcall *JFreeType)     (J);
...
int _stdcall Jwd(J jt, int x, A parg, A* pres, C* loc);
