...
static F1(jttrc){PLOG1;A bot,p,*v,x,y;B b;C*bv,c,ul,ll,*pv;I j,k,m,*s,xn,*xv,yn,*yv;
 RZ(w);
 s=AS(w); v=AAV(w);
...
static F1(jtgraft){PLOG1;A p,q,t,*u,x,y,z,*zv;C*v;I d,j,k,m,n,*pv,*s,xn,*xv,yn,*yv;
 RZ(t=trc(w)); u=AAV(t);
...
static F2(jttroot){PLOG2;A t,x;B b;C*u,*v;I j=0,k=0,m,n,*s;
 RZ(a&&w);
...
static F1(jttleaf){PLOG1;A t,z;C*v;I n,*s;
 RZ(w);
 n=AN(w);
...
static F1(jttconnect){PLOG1;A*wv,x,y,z;B b,d;C c,*u,*xv,*yv,*zv;I e,i,j,m,n,p,q,zn;
 RZ(w);
...
static F1(jttreach){PLOG1;R troot(scc('0'),graft(ope(every(w,0L,jttrr))));}

static F1(jttrr){PLOG1;PROLOG;A fs,gs,hs,s,t,*x,z;B ex,xop;C id;I fl,*hv,m;V*v;
 RZ(w);
...
F1(jttrep){PLOG1;PROLOG; EPILOG(tconnect(troot(mtv,trr(w))));}
