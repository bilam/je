...
static F1(jtdrr){PLOG1;PROLOG;A df,dg,fs,gs,hs,*x,z;B b,ex,xop;C c,id;I fl,*hv,m;V*v;
 RZ(w);
 if(AT(w)&NOUN)R w;
...
F1(jtdrep){PLOG1;A z=drr(w); R z&&AT(z)&BOX?z:ravel(box(z));}


F1(jtaro){PLOG1;A fs,gs,hs,s,*u,*x,y,z;B ex,xop;C id;I*hv,m;V*v;
 RZ(w);
 if(FUNC&AT(w)){
...
F1(jtarep){PLOG1;R box(aro(w));}


static F1(jtfxchar){PLOG1;A y;C c,d,id,*s;I m,n;
 n=AN(w);
...
F1(jtfx){PLOG1;A f,fs,g,h,p,q,*wv,y,*yv;C id;I m,n=0,wd,yd;
 RZ(w);
...
F2(jtunparsem){PLOG2;A h,*hv,dc,ds,mc,ms,z,*zu,*zv;I dn,m,mn,n,p;V*wv;
 RZ(a&&w);
...
static F2(jtxrep){PLOG2;A h,*hv,*v,x,z,*zv;CW*u;I i,j,n,q[3],*s;V*wv; 
 RZ(a&&w);
...
F1(jtarx){PLOG1;F1RANK(0,  jtarx,0); R arep(  symbrdlock(nfb(w)));}
F1(jtdrx){PLOG1;F1RANK(0,  jtdrx,0); R drep(  symbrdlock(nfb(w)));}
F1(jttrx){PLOG1;F1RANK(0,  jttrx,0); R trep(  symbrdlock(nfb(w)));}
F1(jtlrx){PLOG1;F1RANK(0,  jtlrx,0); R lrep(  symbrdlock(nfb(w)));}
F1(jtprx){PLOG1;F1RANK(0,  jtprx,0); R prep(  symbrdlock(nfb(w)));}

F2(jtxrx){PLOG2;F2RANK(0,0,jtxrx,0); R xrep(a,symbrdlock(nfb(w)));}
