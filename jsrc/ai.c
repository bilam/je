...
static F2(jtfong){PLOG2;A f;C c;V*v;
 RZ(a&&w);
...
static F1(jtinvfork){PLOG1;A f,fi,g,gi,h,k;B b,c;V*v;
 RZ(w);
...
static DF1(jtexpandf){PLOG1;A f; RZ(w&&self); f=VAV(self)->f; R expand(VAV(f)->f,w);}

static DF1(jtexpandg){PLOG1;A f,g,z;V*v;
 RZ(w&&self);
...
static F2(jtdiag){PLOG2;I d,k,m,p,r,t,*v;
 RZ(a&&w);
...
static F1(jtbminv){PLOG1;A*wv,x,z=w;I i,j,m,r,*s,t=0,*u,**v,*y,wn,wr,*ws,wd;
 RZ(w);
 ASSERT(0,EVNONCE);
...
static F1(jtinvamp){PLOG1;A f,ff,g,h,*q,x,y;B nf,ng;C c,d,*yv;I n;V*u,*v;
 RZ(w);
 v=VAV(w);
 f=v->f; nf=!!(NOUN&AT(f));
 g=v->g; ng=!!(NOUN&AT(g));
...
F1(jtinv){PLOG1;A f,ff,g;B b,nf,ng,vf,vg;C id,*s;I p,q;V*v;
 RZ(w);
...
static F1(jtneutral){PLOG1;A x,y;B b;V*v;
 RZ(w);
 v=VAV(w);
...
F1(jtiden){PLOG1;A f,g,x=0;V*u,*v;
