...
static F1(jtfmtbfc){PLOG1;A*u,z;B t;C c,p,q,*s,*wv;I i,j,m,n;
 RZ(w); 
...
static F1(jtfmtparse){PLOG1;A x,z,*zv;B ml[2+NMODVALS],mod,t;C c,*cu="srqpnmdblc",*cu1="?bdmnpqrs",d,*s,*wv;
...
F2(jtfmt02){PLOG2;I mode; R fmtxi(a,w,0,&mode);} /* 8!:0 dyad */

F2(jtfmt12){PLOG2;A z;I mode,r,*s;
 RZ(a&&w);
...
F2(jtfmt22){PLOG2;A z;I mode,r,*s;
 RZ(a&&w);
 ASSERT(2>=AR(w), EVRANK);
...
F1(jtfmt01){PLOG1;RZ(w); R fmt02(AR(w)?reshape(sc(*(AS(w)+AR(w)-1)),ace):ace,w);} /* 8!:0 monad */
F1(jtfmt11){PLOG1;RZ(w); R fmt12(AR(w)?reshape(sc(*(AS(w)+AR(w)-1)),ace):ace,w);} /* 8!:1 monad */
F1(jtfmt21){PLOG1;RZ(w); R fmt22(AR(w)?reshape(sc(*(AS(w)+AR(w)-1)),ace):ace,w);} /* 8!:2 monad */
