...
#define ASSERTN(b,e,nm) {if(unlikely(!(b))){jt->curname=(nm); jsignal(e); R 0;}};  // signal error, overriding the running name with a different one
#define ASSERTNGOTO(b,e,nm,lbl) {if(unlikely(!(b))){jtjsignale(jt,(e),(nm),0); goto lbl;}}  // same, but without the exit
#define ASSERTPYX(e,line)   {jtjsignale(jt,(e),line,0); R 0;}
#define ASSERTSYSCOMMON(b,s,stmt)  {if(unlikely(!(b))){fprintf(stderr,"system error: %s : file %s line %d\n",s,__FILE__,__LINE__); jsignal(EVSYSTEM); jtwri(jt,MTYOSYS,"",(I)strlen(s),s); stmt}}
...
#define ASSERTMTV(w)    {ASSERT(1==AR(w),EVRANK); ASSERT(!AN(w),EVLENGTH);}
#define ASSERTN(b,e,nm) {if(unlikely(!(b))){fprintf(stderr,"error code: %i : file %s line %d\n",(int)(e),__FILE__,__LINE__); jtjsignale(jt,(e),(nm),0); R 0;}}  // signal error, overriding the running name with a different one
#define ASSERTNGOTO(b,e,nm,lbl) {if(unlikely(!(b))){fprintf(stderr,"error code: %i : file %s line %d\n",(int)(e),__FILE__,__LINE__); jtjsignale(jt,(e),(nm),0); goto lbl;}}  // same, but without the exit
#define ASSERTPYX(e,line)   {fprintf(stderr,"error code: %i : file %s line %d\n",(int)(e),__FILE__,__LINE__); jtjsignale(jt,(e),line,0); R 0;}
#define ASSERTSYSCOMMON(b,s,stmt)  {if(unlikely(!(b))){fprintf(stderr,"system error: %s : file %s line %d\n",s,__FILE__,__LINE__); jsignal(EVSYSTEM); jtwri(jt,MTYOSYS,"",(I)strlen(s),s); stmt}}
#define ASSERTSYS(b,s)  ASSERTSYSCOMMON(b,s,R 0;)
#define ASSERTSYSGOTO(b,s,lbl)  ASSERTSYSCOMMON(b,s,goto lbl;)
#define ASSERTSYSV(b,s,stmt) ASSERTSYSCOMMON(b,s,stmt)
