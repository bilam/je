...
static DF1(jteach1){PLOG1;DECLF; R every (  w,fs,f1);}
static DF2(jteach2){PLOG2;DECLF; R every2(a,w,fs,f2);}

DF2(jteachl){PLOG2;RZ(a&&w&&self); R rank2ex(a,w,self,-1L, RMAX,VAV(self)->f2);}
DF2(jteachr){PLOG2;RZ(a&&w&&self); R rank2ex(a,w,self,RMAX,-1L, VAV(self)->f2);}

static DF1(jtunder1){PLOG1;DECLFG; R df1(  w,atop(inv(gs),amp(fs,gs)));}
static DF2(jtunder2){PLOG2;DECLFG; R df2(a,w,atop(inv(gs),amp(fs,gs)));}

static DF1(jtundco1){PLOG1;DECLFG; R df1(  w,atop(inv(gs),ampco(fs,gs)));}
static DF2(jtundco2){PLOG2;DECLFG; R df2(a,w,atop(inv(gs),ampco(fs,gs)));}


static DF1(jtunderai1){PLOG1;DECLF;A x,y,z;B b;I j,n,*u,*v;UC f[256],*wv,*zv;
 RZ(w);
...
F2(jtunder){PLOG2;A x;AF f1,f2;B b,b1;C c;I m,r;V*u,*v;
 ASSERTVV(a,w);
