...
DFI2(jtexpn2  ){IARG2 F12IP0; if(unlikely(((((I)AR(w)-1)&SGNIF(AT(w),FLX))<0)))if(unlikely(0.5==DAV(w)[0]))R sqroot(a);  R jtatomic2(jtfg,afg,wfg,self);}  // use sqrt hardware for sqrt.  Only for atomic w. 
