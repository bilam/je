...
static DF1(jtlcapco1){PLOG1;A z;I m;V*v=VAV(self); 
 RZ(w); 
...
static DF2(jtlcapco2){PLOG2;A z;I l,r;V*v=VAV(self);
 RZ(a&&w);
...
static DF1(jtscapco1){PLOG1;A x,z=0;I m;V*v=VAV(self);
 RZ(w); 
 m=jt->lmon; jt->lmon=efflev(0L,v->h,w);
...
static DF2(jtscapco2){PLOG2;A x,z=0;I l,r;V*v=VAV(self); 
 RZ(a&&w); 
 l=jt->lleft;  jt->lleft =efflev(1L,v->h,a);
 r=jt->lright; jt->lright=efflev(2L,v->h,w);
 GA(x,INT,100,1,0); jt->scv=AV(x); jt->sca=x; jt->scn=0; 
...
F2(jtlcapco){PLOG2;R lsub(CLCAPCO,a,w);}
F2(jtscapco){PLOG2;R lsub(CSCAPCO,a,w);}
